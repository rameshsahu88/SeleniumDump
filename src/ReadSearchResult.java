					...
					for (int x = 0; x < cntDay.size(); x++) {
...

						{
							...
							if (userday == selectdays) {
								...
								List<WebElement> SearchRecords = driver.findElements(By.xpath(
										"//div[@class='makeFlex align-items-center gap-x-10 airline-info-wrapper']/div/p"));
								System.out.println("Total records are " + SearchRecords.size());
								int Xaxis = 0, yaxis = 0;
								for (int recCnt = 0; recCnt < SearchRecords.size(); recCnt = recCnt + 2) {
									int indPrcTm = recCnt-1;
									Act.scrollByAmount(Xaxis + 2, yaxis + 100).build().perform();
									System.out.println("Flight names are - " + driver.findElements(By.xpath(
											"//div[@class='makeFlex align-items-center gap-x-10 airline-info-wrapper']/div/p"))
											.get(recCnt).getText()
											+ driver.findElements(By.xpath(
													"//div[@class='makeFlex align-items-center gap-x-10 airline-info-wrapper']/div/p"))
													.get(recCnt + 1).getText()
											+ " And price is - "
											+ driver.findElements(By.xpath("//div[@class='blackText fontSize18 blackFont white-space-no-wrap clusterViewPrice']")).get(recCnt).getText()
											+ " and timing of flight is "
											+ driver.findElements(By.xpath("//p[@class='appendBottom2 flightTimeInfo']/span")).get(recCnt).getText());
								}
